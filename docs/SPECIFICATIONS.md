...
`/`、`POST /save`、`POST /clear`、`/weather`、`/api/grafana/latest`、未定義パスを含むすべての HTTP リクエストで Digest 認証を先に検証する。POST は認証後に既存の CSRF トークンを検証する。保存済み Wi-Fi パスワード、Ingest Key、Grafana API Key は HTML に含めず、各入力が空の場合は保存値を維持する。

設定削除では Wi-Fi、湿度補正、New Relic、Grafana、気象データを消去するが、アクセスキーは維持する。STA 側アクセスは HTTP 本文を暗号化しないため、信頼できる LAN でのみ使用する。
...
| Grafana API Key | Password | 1-256 characters when newly entered | EEPROM |

Ingest Key と Grafana API Key の入力が空の場合、対応する保存済みキーを維持する。保存済みキーは HTML の `value` に設定せず、設定済みかどうかだけを表示する。deviceId または location が未設定でも Wi-Fi 設定は保存できるが、外部メトリクス送信は行わない。
...
現在の保存構造へ `grafanaApiKey` を末尾追加して magic を更新する。直前のアクセスキー対応構造を含む既存形式から SSID、パスワード、湿度補正値、deviceId、location、Ingest Key、海面気圧、対象日、標高、アクセスキーを維持して移行する。
...
| Content-Type | `application/json` |
| Authentication | Basic: fixed Instance ID and EEPROM Grafana API Key |
| Timeout | 5000 ms |
| Success | HTTP status 200-299 |

OTLP JSON は `resourceMetrics`、`scopeMetrics`、8個の gauge metric、各 metric の1個の data point で構成する。`timeUnixNano` は NTP 同期済み Unix 秒を10億倍した10進文字列とする。各 data point の attributes に deviceId、location、sensor を設定する。Grafana Cloud 保存時にはメトリクス名の `.` が `_` へ変換される。

TLS では Grafana Cloud endpoint の信頼チェーンを検証する。`setInsecure()` は使用しない。

## 最新 Grafana ペイロード Web 表示

Grafana 送信条件を満たして OTLP JSON の生成に成功した時点で、送信用の固定長バッファを最新ペイロードとして扱う。次回の生成で同じバッファを上書きし、EEPROM やファイルシステムには保存しない。HTTP 送信の成否にかかわらず、実際に送信を試行したJSONを保持する。

| Route | Response | Authentication |
| ----- | -------- | -------------- |
| `GET /weather` | Arc Gauge HTML | HTTP Digest |
| `GET /api/grafana/latest` | Latest OTLP JSON; `404` before first payload | HTTP Digest |

Arc Gauge 画面は初回表示時と1分間隔で JSON endpoint を取得する。ブラウザ標準の `JSON.parse()` で8個の OTLP gauge data point を読み取り、外部ライブラリを使わず SVG の半円で描画する。範囲外の値は表示値を維持し、Arc の長さだけを0〜100%へ制限する。

| Metric | Label | Unit | Minimum | Maximum |
| ------ | ----- | ---- | ------- | ------- |
| `home.weather.temperature` | 気温 | °C | -10 | 40 |
| `home.weather.humidity` | 相対湿度 | %RH | 0 | 100 |
| `home.weather.absoluteHumidity` | 絶対湿度 | g/m³ | 0 | 30 |
| `home.weather.pressure` | 気圧 | hPa | 950 | 1050 |
| `home.weather.altitude` | 推定標高 | m | -100 | 500 |
| `home.weather.vpd` | VPD | kPa | 0 | 3 |
| `home.weather.wbgt` | WBGT | °C | 0 | 40 |
| `home.weather.discomfortIndex` | 不快指数 | index | 40 | 100 |

WBGT の Arc は25°C未満を緑、25°C以上28°C未満を黄、28°C以上31°C未満を橙、31°C以上を赤とする。他の Arc は共通の青を使用する。各 Gauge は数値、単位、最小値、最大値を文字でも表示する。
...
標高が無効な場合は、全メトリクス有限値チェックにより New Relic と Grafana Cloud への送信を行わない。
