...
`/`、`POST /save`、`POST /clear`、未定義パスを含むすべての HTTP リクエストで Digest 認証を先に検証する。POST は認証後に既存の CSRF トークンを検証する。保存済み Wi-Fi パスワード、Ingest Key、Grafana API Key は HTML に含めず、各入力が空の場合は保存値を維持する。
